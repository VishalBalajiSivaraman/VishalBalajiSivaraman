...
<!-- - **RaZORNet: Rotation-Aware Zero-Shot Registration Network** (**Role**: ***Principle Investigator***): is a **zero-shot image registration network** designed to align image pairs with **large relative rotations** using **rotation-equivariant features** extracted from the depths of a **novel hybrid foundational model SQNet**. RaZORNet outperforms current **state-of-the-art methods**, achieving **state-of-the-art performance** on **two newly introduced large-scale roto-registration datasets**. Further details on the datasets, model architecture, pretraining, loss function and design will be shared upon paper release. -->
